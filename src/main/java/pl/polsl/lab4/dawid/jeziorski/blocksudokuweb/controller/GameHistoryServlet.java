 ...
 * @version 1.0
 */
@WebServlet(name = "previous-games", urlPatterns = {"/previous-games"})
public class GameHistoryServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // Get IDs of user past games
        List<Integer> pastGames = gameHistory.getPastGames(name);
       
        // Print information for user on website
        try (PrintWriter out = response.getWriter()) {
            viewer.printStart(out);
            
            if(!pastGames.isEmpty()){
                viewer.printPastGames(out, pastGames);
            }
